    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">{{ __('ListTitle', ['name' => __(\Illuminate\Support\Str::plural('BrandPhoto')) ]) }}</h3>

                <div class="px-2 mt-4">

                    <ul class="breadcrumb mt-3 py-3 px-4 rounded">
                        <li class="breadcrumb-item"><a href="@route(getRouteName().'.home')" class="text-decoration-none">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __(\Illuminate\Support\Str::plural('BrandPhoto')) }}</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if(getCrudConfig('BrandPhoto')->create && hasPermission(getRouteName().'.brandphoto.create', 1, 0))
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.brandphoto.create')" class="btn btn-success">{{ __('CreateTitle', ['name' => __('BrandPhoto') ]) }}</a>
                        </div>
                        @endif
                        @if(getCrudConfig('BrandPhoto')->searchable())
                        <div class="col-md-4">
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover table-striped">
                    ...
                    <tbody>
                        @foreach($brandphotos as $brandphoto)
                            @livewire('admin.brand-photo.single', [$brandphoto], key($brandphoto->id))
                        @endforeach
                    </tbody>
                </table>
            </div>
            <div class="m-auto pt-3 pr-3">
                {{ $brandphotos->appends(request()->query())->links() }}
            </div>
