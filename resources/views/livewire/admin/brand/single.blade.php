<tr x-data="{ modalIsOpen : false }">
    ...
    <td class="">{{ $brand->url }}</td>
    <td class="">{{ $brand->store_id }}</td>
    <td class="">{{ $brand->store->name }}</td>
    
    @if(getCrudConfig('Brand')->delete or getCrudConfig('Brand')->update)
        <td>

            @if(getCrudConfig('Brand')->update && hasPermission(getRouteName().'.brand.update', 1, 0, $brand))
                <a href="@route(getRouteName().'.brand.update', $brand->id)" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('Brand')->delete && hasPermission(getRouteName().'.brand.delete', 1, 0, $brand))
