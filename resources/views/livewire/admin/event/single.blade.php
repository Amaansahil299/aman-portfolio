<tr x-data="{ modalIsOpen : false }">
    ...
    @if(getCrudConfig('Event')->delete or getCrudConfig('Event')->update)
        <td>
            ...
            @if(getCrudConfig('Event')->update && hasPermission(getRouteName().'.event.update', 1, 0, $event))
                <a href="@route(getRouteName().'.event.update', $event->id)" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('Event')->delete && hasPermission(getRouteName().'.event.delete', 1, 0, $event))
